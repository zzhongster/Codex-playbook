...
**证据标记：`source-project-observation`；案例边界：`single-project`**
...
[gjpERP Delphi ERP 逆向案例](../case-studies/gjperp-delphi-erp.md)现已用限定稳定 ID、版本/环境等输入身份、来源提交与 Git blob OID 绑定脱敏来源，并逐项区分 `project-validated` 和 `proposed` 方法。该证据仍只来自单一项目：同一 ERP 内的多个模块或业务集群不构成跨项目验证，不能据此晋升为 Pattern，也不能证明其他 Delphi 产品采用相同拓扑。本 Delphi 分册仍为 `proposed`；只有案例表中明确标为 `project-validated` 的方法，才能在该案例公开的边界内作单项目陈述。
...
- **白盒：** 拥有完整且已获授权读取的源码时，选择[白盒访问轨道](../access-tracks/white-box.md)；不以构建材料是否存在或获准作为白盒轨道的前置条件。构建文件/材料、构建执行和运行观察是相互独立的可选证据与权限面，缺失或未获准时分别明确记录；源码读取授权不自动授权构建或运行，也不能跳过 G0、版本绑定或受治理的非运行分支。
...
运行实验属于[运行时实验](../core/runtime-experiments.md)，必须绑定当前目标、动作和环境的 `ART-P0-AUTH`，且 `ART-G0-AUTH` 的 `verdict` 为 `pass`。仅在隔离的 32 位遗留 Windows 会话中运行已选客户端、安装程序及实际遇到的依赖；位数、OS build、区域/代码页、组件、输入及实际使用的分支身份必须与任务包一致。
...
每次实验只记录实际使用的数据或外部依赖身份：数据库分支记录数据库副本稳定 ID，文件分支记录文件/目录稳定 ID 与哈希，远程分支记录脱敏端点/服务稳定 ID，设备分支记录设备稳定 ID，无数据层分支显式记录 `no-data-layer`；组合分支逐项记录。不要求每次实验记录数据库身份，也不得用空数据库字段掩盖实际使用的文件、服务或设备。

每次实验还记录可执行文件/模块哈希、用户角色、配置、入口、动作、观察面、预期副作用、实际副作用、清理和终态。失败也保留为证据；任一实际分支身份漂移、数据库连接到非副本、捕获中断、未知写入、隔离削弱或无法恢复时立即停止，不沿用同一运行记录继续试错。
...
- 授权、身份、版本、实际分支目标或隔离状态漂移时立即停止；必须重新授权、重新绑定身份并新建运行记录。
