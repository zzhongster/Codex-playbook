...
version_scope:
  ...
  scope_or_module: "scope:template.replace-me"
  environment_id: "environment:template.replace-me"
...
validation_method:
  ...
  description: "REPLACE_WITH_DETERMINISTIC_REBUILD_AND_REVIEW_METHOD"
  reproducible: true
...
| dimension | denominator | numerator | unknown_count | conflicting_count | excluded_count | P0/P1/P2 |
| --- | --- | --- | --- | --- | --- | --- |
...
九类维度分别计算，不把不同单位直接相加；每维使用互斥且穷尽的桶，并强制 `denominator = numerator + unknown_count + conflicting_count + excluded_count`。未知队列记录影响、下一动作、责任人、期限和重开条件。
...
G5 的合法组合是封闭集合：运行分支开始或失败时为 `runtime + pending/fail + required`，获得完整运行确认时才是 `runtime + pass + confirmed`；获批静态分支只能是 `approved-static + pending/pass/fail + unavailable-with-approved-static-ceiling`。静态分支即使仍在等待门禁判定，也必须在 `static_non_runtime_coverage` 持续引用 `ART-P5-RUNTIME-GAP` 的稳定 ID/哈希，并用确定性规则 `gap_count = runtime.unknown_count + runtime.conflicting_count` 保存所有未获运行确认的项目；不能以 `pending` 或 G5 通过清零、隐藏运行覆盖缺口。运行分支保持 gap reference 为 `null`、gap count 为 `0`，且只有 runtime 的 unknown/conflicting 都为零时才能 `pass`。
